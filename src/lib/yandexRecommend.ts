...
export async function recommendWithYandex(
  ...
  // По умолчанию lite, а не старшая модель. На замерах (10 запросов подряд,
  // одинаковый объём ответа) старшая давала медиану 5.5 с, но два выброса
  // за 25 секунд и один за 36 — это очередь на стороне Яндекса, а не более
  // долгая генерация. Lite уложилась в 2.9–4.6 с без единого выброса при
  // сопоставимом качестве подборок. Переопределяется через YANDEX_MODEL.
  const model =
    process.env.YANDEX_MODEL || `gpt://${folderId}/yandexgpt-lite/latest`;
