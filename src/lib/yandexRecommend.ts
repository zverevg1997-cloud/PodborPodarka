...
export async function recommendWithYandex(
  ...
  // Выбрана по замерам на одинаковых анкетах. yandexgpt-lite пересказывала
  // анкету («папа любит рыбалку, поэтому удочка пригодится»), старшая
  // yandexgpt уходила в очередь на 25–36 секунд, qwen3 давала лучшие идеи,
  // но за 20+ секунд и с выдуманными товарами. aliceai-llm отвечает за 3–4
  // секунды, ссылается на реально существующие вещи и отыгрывает повод.
  // Переопределяется через YANDEX_MODEL.
  const model =
    process.env.YANDEX_MODEL || `gpt://${folderId}/aliceai-llm/latest`;
